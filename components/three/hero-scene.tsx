...
// Satellite/Rocket component that orbits on a circular trajectory
function AerospaceVehicle({ radius, speed, color, offset }: { radius: number, speed: number, color: string, offset: number }) {
    ...
    useFrame((state) => {
        ...
        // Point the nose forward (tangent to the circle)
        groupRef.current.rotation.z = t;
    });

    return (
        <group ref={groupRef}>
            {/* Aerodynamic shape representing rocket/drone */}
            <mesh rotation={[0, 0, -Math.PI / 2]}>
                <coneGeometry args={[0.1, 0.5, 4]} />
                <meshBasicMaterial color={color} wireframe />
            </mesh>
            {/* Glowing thruster core at the back */}
...
// A wired ring representing the flight trajectory or orbit
...
// A single spinning ring mimicking an IMU Gimbal / Gyroscope
function GimbalRing({ radius, axis, speed }: { radius: number, axis: 'x' | 'y' | 'z', speed: number }) {
    ...
    return (
        <mesh ref={ringRef}>
            ...
            {/* Inner glowing edge for tech aesthetic */}
            <mesh>
...
// The central "Web Server / Brain" of the system + Gyroscope
function CoreSystem() {
    ...
    return (
        <group>
            <Float speed={2} rotationIntensity={0.2} floatIntensity={0.5}>
                ...
                {/* Solid glowing processing center */}
                <mesh>
            ...
            {/* Avionics Gimbal Rings representing Hardware Control (IMU) */}
...
// The master assembly containing the core and orbiting systems
function AvionicsWebArchitecture() {
    ...
    useFrame((state, delta) => {
        if (systemRef.current) {
            // The whole complex slowly rotates over time
    ...
    // Shift model to right side of screen on desktop, keep centered on mobile.
    ...
    return (
        <group position={[positionX, 0, 0]} scale={[overallScale, overallScale, overallScale]}>
            <group ref={systemRef}>
                ...
                {/* Inner Orbit: Data Packets / Web Traffic */}
                <group rotation={[Math.PI / 3, Math.PI / 4, 0]}>
                    <OrbitPath radius={2.0} />
                    <Trail width={1.5} color="#00e5ff" length={4} decay={1} local>
                        <AerospaceVehicle radius={2.0} speed={0.8} color="#00e5ff" offset={0} />
                    </Trail>
                </group>

                {/* Middle Orbit: Avionics Telemetry Drones */}
                <group rotation={[Math.PI / 2.5, 0, Math.PI / 6]}>
                    <OrbitPath radius={2.8} />
                    <Trail width={2} color="#ff0055" length={6} decay={1.5} local>
                        <AerospaceVehicle radius={2.8} speed={0.4} color="#ff0055" offset={Math.PI} />
                    </Trail>
                </group>

                {/* Outer Orbit: Long Range Rocket Trajectory */}
                <group rotation={[-Math.PI / 6, -Math.PI / 4, 0]}>
                    <OrbitPath radius={3.8} />
                    <Trail width={3} color="#ffaa00" length={10} decay={1.5} local>
...
export default function HeroScene() {
    ...
    return (
        <div className="w-full h-full relative">
            {/* Fallback pattern for mobile / loading state */}
            <div
                className={`absolute inset-0 bg-[#0a0a0a] transition-opacity duration-1000 ${isMobile ? "opacity-100" : "opacity-0"}`}
                style={{
